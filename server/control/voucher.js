...
var getOpenID = async (ctx, next) => {
  ...
  let ret = await new Promise(
    function(resolve, reject){
      https.get("https://api.weixin.qq.com/sns/jscode2session?appid=" + fs.readFileSync('../../' + dt.app + '/appid.txt') + 
      "&secret=" + fs.readFileSync('../../' + dt.app + '/appsecret.txt') + '&js_code=' + dt.code + "&grant_type=authorization_code",
      function(res){
       var datas = [];  
       var size = 0;  
